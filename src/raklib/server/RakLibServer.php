...
class RakLibServer extends \Thread {
	 ...
	 * @throws \Exception
	 */
	public function __construct(\ThreadedLogger $logger, \ClassLoader $loader, $port, protected $interface = "0.0.0.0") {
		$this->port = (int) $port;
		...
		for ($i = (int) $start; isset($trace[$i]); ++$i, ++$j) {
			...
			if (isset($trace[$i]["args"]) || isset($trace[$i]["params"])) {
				...
				foreach ($args as $name => $value) {
					$params .= (is_object($value) ? $value::class . " " . (method_exists($value, "__toString") ? $value->__toString() : "object") : gettype($value) . " " . @strval($value)) . ", ";
				}
			}
